package com.exp.ucmp.contract.utils;
...
import com.exp.ucmp.contract.web.Main;
...
public class WorderToNewWordUtils {
    ...
    public static void changeTable(CustomXWPFDocument document, Map<String, Object> textMap, List<Object> mapList,int[] placeList){
        ...
        LOGGER.info("获取表格对象集合="+tables.size());
        LOGGER.info("获取表格对象集合="+tables.get(0).getRow(0).getCell(0).getText());
        //循环所有需要进行替换的文本，进行替换
        ...
        for (int i = 0; i < tables.size(); i++) {
            ...
            XWPFTable table = tables.get(i);
            LOGGER.info("I="+i);
            LOGGER.info("placeList[index]="+placeList[index]);
            if(placeList[index]==i){
                List<String[]> list = (List<String[]>) mapList.get(index);
                LOGGER.info("list="+i+"=="+JsonBeanUtil.beanToJson(list));
                //第二个表格使用daList，插入数据
            ...
            LOGGER.info("==table=="+table.getRow(1).getCell(1).getText());
        }
    }
    /**
     * 遍历表格
...
//        	LOGGER.info("textSet="+textSet.getKey());
            //匹配模板与替换值 格式${key}
            String key = textSet.getKey();
            if(value.indexOf(key)!= -1){
            	if(textSet.getValue()!=null){
            		valu = value.replace(key, textSet.getValue().toString());
            	}else{
            		valu = value.replace(key, "");
            	}
            }
        }
        return valu;
    ...
    public static Object changeValueNew(String value, Map<String, Object> textMap){
        Set<Map.Entry<String, Object>> textSets = textMap.entrySet();
        for (Map.Entry<String, Object> textSet : textSets) {
//        	LOGGER.info("textSet="+textSet.getKey());
            //匹配模板与替换值 格式${key}
