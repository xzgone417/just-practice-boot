...
public class DaoWrapper extends Wrapper{
	...
	public DaoWrapper(IntrospectedTable arg0, GeneratorConfig config) {
		...
		for(IntrospectedColumn column : arg0.getAllColumns()) {
			...
			if(!column.getFullyQualifiedJavaType().getFullyQualifiedNameWithoutTypeParameters().startsWith("java.lang.")&&!column.getFullyQualifiedJavaType().getFullyQualifiedNameWithoutTypeParameters().startsWith("java.util.")) {
